...
class UserSchema(Schema):
    ...
    weight = fields.Decimal(as_string=True, places=1, required=True, validate=validate.Range(min=0))
    height = fields.Decimal(as_string=True, places=1, required=True, validate=validate.Range(min=0))
    gender = fields.Str(required=True, validate=validate.OneOf(["Male", "Female"]))
    activity_level = fields.Str(required=True, validate=validate.OneOf(
        ["Sedentary", "Lightly Active", "Moderately Active", "Very Active", "Super Active"]
    ))
    goal = fields.Str(required=True, validate=validate.OneOf(["Cut", "Maintain", "Bulk"]))
